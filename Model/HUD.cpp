...
void CHUD::Update(float elapsedMs)
{
	...
	_level = unsigned int(_mario->_momentum * POWER_LIMIT / _mario->MOMENTUM_LIMIT);
}

void CHUD::Render()
{
	_sprites[SPR_HUD_FRAME]->Render(FRAME_X, FRAME_Y, true);
	if (!_mario->_fullspeed)
	{
		if (_level > 1)	_sprites[SPR_ARROW]->Render(LEVEL1_X, LEVEL1_Y, true);
		if (_level > 2)	_sprites[SPR_ARROW]->Render(LEVEL2_X, LEVEL2_Y, true);
		if (_level > 3)	_sprites[SPR_ARROW]->Render(LEVEL3_X, LEVEL3_Y, true);
		if (_level > 4)	_sprites[SPR_ARROW]->Render(LEVEL4_X, LEVEL4_Y, true);
		if (_level > 5)	_sprites[SPR_ARROW]->Render(LEVEL5_X, LEVEL5_Y, true);
		if (_level > 6)	_sprites[SPR_ARROW]->Render(LEVEL6_X, LEVEL6_Y, true);
	}
	else
	{
		if (_blink)
		{
			_sprites[SPR_ARROW]->Render(LEVEL1_X, LEVEL1_Y, true);
			_sprites[SPR_ARROW]->Render(LEVEL2_X, LEVEL2_Y, true);
			_sprites[SPR_ARROW]->Render(LEVEL3_X, LEVEL3_Y, true);
			_sprites[SPR_ARROW]->Render(LEVEL4_X, LEVEL4_Y, true);
			_sprites[SPR_ARROW]->Render(LEVEL5_X, LEVEL5_Y, true);
			_sprites[SPR_ARROW]->Render(LEVEL6_X, LEVEL6_Y, true);
			_sprites[SPR_POWER]->Render(POWER_X, POWER_Y, true);
		}
	}
}
